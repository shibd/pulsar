...
void ConsumerImpl::connectionOpened(const ClientConnectionPtr& cnx) {
    ...
    cnx->sendRequestWithId(cmd, requestId)
        .addListener(
            std::bind(&ConsumerImpl::handleCreateConsumer, get_shared_this_ptr(), cnx, std::placeholders::_1));
}
...
void ConsumerImpl::notifyBatchPendingReceivedCallback(const BatchReceiveCallback& callback) {
    auto messages =
        std::make_shared<MessagesImpl>(batchReceivePolicy_.getMaxNumMessages(), batchReceivePolicy_.getMaxNumBytes());
    Message peekMsg;
...
void ConsumerImpl::closeAsync(ResultCallback callback) {
    ...
    if (callback) {
        ...
        future.addListener(
            std::bind(&ConsumerImpl::handleClose, get_shared_this_ptr(), std::placeholders::_1, callback, ptr));
    }
...
bool ConsumerImpl::hasEnoughMessagesForBatchReceive() const {
    LOG_INFO("calc msg, current: size:" << incomingMessages_.size()
                                        << " bytes size:" << incomingMessagesSize_);
